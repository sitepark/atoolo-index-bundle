...
use Atoolo\Index\Dto\Indexer\IndexerParameter;
...
/**
 * Indexes the resources of the resource tree into an index.
 *
 * Resources are loaded via the indexer, mapped to an IndexDocument and
 * then transferred to the index target.
 *
 ...
 *    available in translated form as PHP files in the file system. Depending
 *    on the target a separate index is used for each language. The PHP files
 *    are therefore assigned to the respective language.
 ...
 *
 * The indexer itself is target agnostic. The concrete target is provided via
 * the {@see IndexService} port, the concrete document via the enricher list
 * of the target.
 */
class InternalResourceIndexer extends AbstractIndexer implements UpdatableIndexer
{
     ...
     * These are then passed to the index target via one request. This
     * method accepts a chunk with all resources that are to be indexed via
     * one request.
     *
     * @param string $processId
